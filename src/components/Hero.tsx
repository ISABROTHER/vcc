...
export default function Hero() {
  ...
  return (
    <section className="relative w-full min-h-[400px] md:h-[550px] flex items-end overflow-hidden font-sans bg-slate-900 group">
        ...
        >
          <div
            ...
            style={{
              ...
              // Centered position ensures the "meat" of the photo is always visible
              backgroundPosition: 'center center', 
              backgroundRepeat: 'no-repeat',
            }}
          />
          
          {/* Refined Gradients: Cleared the center so the image pops, dark only behind text */}
      ...
      {/* Navigation Arrows - Pushed to absolute edges to not block view */}
      <button 
        onClick={prevSlide}
        className="absolute left-2 md:left-4 top-1/2 -translate-y-1/2 z-30 p-2 md:p-3 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-white hover:bg-white/30 transition-all hover:scale-110 active:scale-95 md:opacity-0 md:group-hover:opacity-100"
      ...
      <button 
        onClick={nextSlide}
        className="absolute right-2 md:right-4 top-1/2 -translate-y-1/2 z-30 p-2 md:p-3 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-white hover:bg-white/30 transition-all hover:scale-110 active:scale-95 md:opacity-0 md:group-hover:opacity-100"
        aria-label="Next slide"
      >
        <ChevronRight className="w-6 h-6 md:w-8 md:h-8" />
      </button>

      {/* Content Container - Lowered padding to move text down and reveal more image */}
      <div className="relative z-10 w-full max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 pb-12 md:pb-16 lg:pb-20 pt-24 pointer-events-none">
        <div 
        ...
        >
          ...
          {/* Headline */}
          <h1 className="text-4xl sm:text-6xl md:text-7xl lg:text-8xl font-bold text-white mb-3 md:mb-5 leading-[0.95] tracking-tight drop-shadow-2xl pointer-events-auto">
      ...
      {/* Slider Dots */}
      <div className="absolute bottom-4 md:bottom-6 left-0 right-0 flex justify-center gap-3 z-20 pointer-events-auto">
